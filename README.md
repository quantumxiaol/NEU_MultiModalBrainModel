...
|指标|旧版集成（cc400+aal+ez）|当前 shared/private 版本|
|-|-|-|
|acc|0.5981060606060605|0.6095741901776385|
|Recall|0.5997984589697886|0.6141155152185194|
|F1|0.5733345386475863|0.5972084398733069|
|AUC|0.6734767942961576|0.6662987071591345|
|time|36.392|44.8488986492157|

当前这版 shared/private 实现相比旧版集成，ACC、Recall 和 F1 都有提升，说明最终分类表现更稳定了一些；但平均 AUC 略低于旧版结果，训练时间也有所增加。因此，这一版更适合表述为“分类指标整体回升，但排序能力（AUC）仍有进一步优化空间”。

这次运行中表现最好的单折为 Fold 3，其结果为：

|指标|Fold 3|
|-|-|
|acc|0.735632183908046|
|Recall|0.7340933191940615|
|F1|0.7343687773795302|
|AUC|0.7857900318133616|

说明这套 shared/private 结构在部分数据划分上已经能够取得明显更好的效果，但不同折之间仍然存在较大的波动，稳定性还需要继续优化。
