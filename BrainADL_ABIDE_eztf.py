...
Data_atlas1 = scio.loadmat('./ABIDEdata/pcc_correlation_871_ez_.mat')  # 更新为dos160数据路径
X = Data_atlas1['connectivity']
Y = np.loadtxt('./ABIDEdata/871_label_ez.txt')  # 更新为dos160标签路径
    ...
    for trainval_index, test_index in kf.split(X, Y):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % 1 == 0:
                ...
                print('Test acc', acc)
        os.makedirs('./modelstfez', exist_ok=True)

        torch.save(model.state_dict(), './modelstfez/' + str(kfold_index) + '.pt')
