...
for ind in range(1):
    ...
    for trainval_index, test_index in kf.split(X, Y):
        ...
        model = SimpleTransformerModel(input_dim, model_dim, num_classes=2, num_heads=num_heads, num_layers=num_layers, dropout=dropout)
        model.to(device)
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % 1 == 0:
                ...
                f1_list.append(f1)
        os.makedirs('./modelstfcc400', exist_ok=True) 
        torch.save(model.state_dict(), './modelstfcc400/' + str(kfold_index) + '.pt')
