...
Data_atlas1 = scio.loadmat('./ABIDEdata/pcc_correlation_871_tt_.mat')  # 更新为dos160数据路径
X = Data_atlas1['connectivity']
Y = np.loadtxt('./ABIDEdata/871_label_tt.txt')  # 更新为dos160标签路径
...
for ind in range(1):
    ...
    for trainval_index, test_index in kf.split(X, Y):
        ...
        torch.save(model.state_dict(), './modelstftt/' + str(kfold_index) + '.pt')
